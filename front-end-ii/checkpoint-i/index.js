...
function validarDescricao (event) {
  if (userDescriptionRef.value.length < 4) {
    errorMessageText.innerHTML = "A descrição deve ter no mínimo 10 caracteres";
...
function validarTitulo (event) {
  if (userTitleRef.value.length < 4) {
    errorMessageTitulo.innerHTML = "A descrição deve ter no mínimo 10 caracteres";
    return false
  }
...
function validateUrl () {
  if (userUrlImageRef.value.trim() === "") {
    errorMessageUrl.innerHTML = "A descrição deve ter no mínimo 10 caracteres";
    return false
